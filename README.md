...
TRPG『ワンダーオブワンダラー』のココフォリア用チャパレ式作成と、判定ログの成功度再計算を行う非公式ファンメイドツールです。
...
- 60%／50%リミットガムの判定値補正と成功度切り捨てに対応
- 結果後MPチャージとして選択したダイスを除外し、成功度を再計算
- 通常、ダーク、シンプル（グレースケール）の3表示モード
- ヘッダーから更新履歴とHelpを確認可能
- サーバー・npm・外部API不要
...
- 本ツールは非公式であり、『ワンダーオブワンダラー』の公式または権利者とは関係ありません。
